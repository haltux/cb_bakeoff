...
import argparse
from config import OML_API_KEY # ===> see README.md <====
...
VW_DATA_DIR = os.path.join(BAKEOFF_BASE_DIR, "vw")
...
if __name__ == '__main__':
    ...
    openml.config.set_cache_directory(os.path.join(BAKEOFF_BASE_DIR, "omlcache"))

    print('loaded openML')

    if not os.path.exists(VW_DATA_DIR):
        os.makedirs(VW_DATA_DIR)
    ...
    for did in sorted(dids):
        ...
        try:
            ...
            print(e)
            continue
        save_vw_dataset(X, y, did, BAKEOFF_BASE_DIR)
